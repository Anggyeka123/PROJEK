...
<head>
    ...
    <title>Tambah Peserta | {{ $tournament->name }}</title>
...
<body class="bg-slate-950 text-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold">Tambah Peserta</h1>
        ...
        <form action="{{ route('tournaments.participants.store', $tournament) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6" id="participantForm">
            @csrf
            ...
            <div class="grid gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-white focus:border-indigo-500 focus:outline-none @error('name') border-red-500 @enderror" required>
                    @error('name')
                ...
                <!-- Logo Tim -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Logo Tim (JPG, JPEG, PNG, WebP) <span class="text-red-500">*</span></label>
                    <input type="file" accept="image/jpeg,image/jpg,image/png,image/webp" name="logo" class="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-white focus:border-indigo-500 focus:outline-none @error('logo') border-red-500 @enderror" required>
                    <p class="text-slate-400 text-xs mt-1">Maksimal 2MB</p>
                    @error('logo')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="non-indonesian-fields" class="grid grid-cols-1 sm:grid-cols-2 gap-6" style="display: none;">
                    ...
                    <div>
                        ...
                        <input type="text" id="non-indo-city" name="city" value="{{ old('city') }}" class="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-white focus:border-indigo-500 focus:outline-none @error('city') border-red-500 @enderror">
                        @error('city')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3">
                ...
                <button type="submit" class="px-5 py-3 bg-indigo-600 hover:bg-indigo-700 rounded-xl text-white font-semibold transition">Simpan Peserta</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function populateCountries() {
            ...
            countries.forEach(c => {
                ...
                opt.value = c;
                opt.textContent = c;
                if (c === '{{ old('country', 'Indonesia') }}') opt.selected = true;
        ...
        function populateProvinces() {
            ...
            Object.keys(indonesianProvinces).forEach(p => {
                ...
                opt.value = p;
                opt.textContent = p;
                if (p === '{{ old('province') }}') opt.selected = true;
        ...
        function onProvinceChange() {
            ...
            indonesianProvinces[prov].forEach(ct => {
                ...
                opt.value = ct;
                opt.textContent = ct;
                if (ct === '{{ old('city') }}') opt.selected = true;
