    ...
    <form action="{{ route('tournaments.updateBracketSettings', $tournament) }}" method="POST" class="space-y-6">
        ...
        @php
            ...
            if (isset($teamsToUse) && !empty($teamsToUse)) {
                $teamNames = array_keys($teamsToUse); // Get position names like A1, B1, A4, B4, etc.
                $rankLabel = (isset($playoffMode) && $playoffMode === 'relegation') ? 'Tim Degradasi' : 'Tim Qualified';
            } else {
        ...
        @if(isset($hasBothOptions) && $hasBothOptions)
            ...
            <div id="bracketPromotionSection" class="bracket-section" data-bracket-mode="promotion">
                @include('tournaments.partials.bracket-section', [
                    'mode' => 'promotion',
        ...
        <div class="bg-slate-900 rounded-xl border border-slate-800 p-6">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    @if(isset($playoffMode) && $playoffMode === 'relegation')
                        <p class="text-sm text-slate-400 mt-2">Sistem playoff degradasi otomatis berdasarkan tim yang terdegradasi dari fase grup. Setiap card memiliki <code>data-match-id</code> dan <code>data-next-match-id</code>, lalu konektor SVG menggambar jalur siku-siku antar card. Jika opsi perebutan tempat ketiga dicentang, babak Third Place akan muncul di Pengaturan Slot Bracket.</p>
                    @else
                        <p class="text-sm text-slate-400 mt-2">Sistem turnamen knockout otomatis berdasarkan jumlah tim yang lolos dari fase grup. Setiap card memiliki <code>data-match-id</code> dan <code>data-next-match-id</code>, lalu konektor SVG menggambar jalur siku-siku antar card. Jika opsi perebutan tempat ketiga dicentang, babak Third Place akan muncul di Pengaturan Slot Bracket dan diisi oleh runner-up semifinal.</p>
                    @endif
                </div>
            </div>
                ...
                <div id="bracketConnectorLayout" class="relative min-w-max">
                    ...
                    <div class="relative flex gap-24 w-full items-start">
                        @foreach($bracketColumns as $columnIndex => $column)
                            <div class="relative flex-shrink-0 w-[220px]" data-final-column="{{ $columnIndex === count($bracketColumns) - 1 ? '1' : '0' }}" style="min-height: {{ count($bracketColumns[0]['matches']) * $rowUnit + $columnHeaderHeight }}px;">
                                <div class="mb-4">
