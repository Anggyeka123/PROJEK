...
<head>
    ...
    <title>Edit Turnamen | Futsal League</title>
...
<body class="bg-slate-950 text-white">
    ...
    <header class="border-b border-slate-800 bg-slate-900 bg-opacity-50 backdrop-blur sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
            <div class="flex items-center gap-3 sm:gap-4 mb-4">
                <a href="{{ route('tournaments.show', $tournament) }}" class="text-slate-400 hover:text-white transition">
                ...
                <div class="min-w-0">
                    ...
                    <h1 class="text-xl sm:text-3xl font-bold truncate">Edit: {{ $tournament->name }}</h1>
                </div>
            </div>
        </div>
    </header>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-12">
        ...
        <div class="bg-slate-900 rounded-xl border border-slate-800 p-4 sm:p-8">
            <form action="{{ route('tournaments.update', $tournament) }}" method="POST" class="space-y-4 sm:space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="date" name="match_date" value="{{ old('match_date', optional($tournament->match_date)->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="division" value="{{ old('division', $tournament->division) }}" placeholder="cth: Senior, Junior, U-16" required
                ...
                <div>
                    ...
                    <input type="text" name="venue" value="{{ old('venue', $tournament->venue) }}" placeholder="cth: Lapangan Sebenarnya, Jl. XX" required
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-slate-700">
                    <a href="{{ route('tournaments.show', $tournament) }}" class="flex-1 py-3 px-4 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-lg transition text-center text-sm sm:text-base">
                        Batal
                    </a>
